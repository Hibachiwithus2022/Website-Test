...
export const HERO_SUBTITLES = [
  ...
  (city, state) => `${city}'s Favorite Private Chef Experience • From $55/Adult`,
...
export const INTRO_VARIANTS = [
  // ── 0: Entertainment-first ─────────────────────────────────────────────────
  {
    ...
    closing:  (city, state) =>
      `We've built the service around one idea: the hibachi experience should be even better at home than at the restaurant. No waiting for a table, no crowded dining room, no rushed turns. Just your people, your space, and a chef who owns the room. That's hibachi at home in ${city} — and it starts at $55 per adult.`,
  },
  // ── 1: Convenience-first ───────────────────────────────────────────────────
  {
    angle:    'convenience',
    ...
    closing:  (city, state) =>
      `Hibachi Connect has run hundreds of events across ${state} — birthday dinners, corporate retreats, graduation parties, bachelorette nights, and ordinary Saturdays that became anything but ordinary. If you can host 10 people in ${city}, we can make it an unforgettable evening. Standard package starts at $55 per adult, $550 minimum.`,
  },
  // ── 2: Craft/food-first ────────────────────────────────────────────────────
  {
    angle:    'craft',
    ...
    closing:  (city, state) =>
      `We serve ${city} and surrounding areas with the same standards we'd hold any professional kitchen to — because that's what your guests deserve. Two protein standard packages start at $55 per adult. Premium and Elite options include filet mignon, lobster tail, and dedicated event coordination for larger gatherings.`,
  },
  // ── 3: Social/memory-first ────────────────────────────────────────────────
  {
    ...
    closing:  (city, state) =>
      `Whether you're celebrating something huge or just making an ordinary weekend extraordinary, private hibachi in ${city} is the kind of thing people request at the next event. Book a certified chef through Hibachi Connect. Starting at $55 per adult, $550 minimum. Setup and cleanup fully included.`,
  },
  // ── 4: Value-first ────────────────────────────────────────────────────────
  {
    angle:    'value',
    ...
    closing:  (city, state) =>
      `The $550 event minimum covers 8–10 guests at the standard rate. For larger groups, add-on proteins (lobster tail, filet, scallops) and premium packages are available. Travel fees may apply outside ${city}'s metro area. Get a free, no-obligation quote today — most ${city} bookings are confirmed within a few hours.`,
  },
]
...
export const WHY_US_VARIANTS = [
  ...
  // 1: Ease-led
  {
    ...
    items: (city) => [
      ...
      { icon: '📋', title: 'No Surprise Pricing',          desc: '$55/adult, $30/child, $550 event minimum. Your quote is locked before the event. No hidden charges, ever.' },
...
export const CLOSING_VARIANTS = [
  ...
  },
  {
    ...
    sub:      (city) => `A certified teppanyaki chef. Premium proteins. Full setup and cleanup. Starting at $55 per adult in ${city}.`,
...
export const SHARED_FAQ_POOL = [
  ...
  },
  {
    ...
    a: 'Absolutely — kids are usually the most enthusiastic guests at hibachi events. Children 12 and under are $30 each. Ages 0–3 eat free. The fire tricks and shrimp tosses are crowd favorites for younger guests. Just let your chef know the age range so they can adjust the show accordingly.',
  },
  {
    pool: 'logistics',
  ...
  },
  {
    ...
    a: 'No hidden fees. Your price is $55/adult, $30/child (12 & under), $550 event minimum. Travel fees may apply for locations more than 30 miles from our nearest chef base. All fees are disclosed and agreed upon before you book — nothing surprises you at the event.',
  },
  {
    pool: 'pricing',
    q: 'What\'s included in the event minimum?',
    a: 'The $550 event minimum covers 8–10 guests at the standard rate and includes the chef, grill, propane, all ingredients, sauces, plates, chopsticks, full setup, and complete cleanup. You simply provide the space and seating.',
  },
  {
    pool: 'experience',
...
const TITLE_TEMPLATES = [
  ...
  (city, state) => `${city} Hibachi at Home — Private Chef from $55 | Hibachi Connect`,
  // 3 — action-oriented
...
const DESC_TEMPLATES = [
  // 0 — service + price + CTA
  (city, state) => `Book a private hibachi chef in ${city}, ${state}. Live teppanyaki at your home from $55/adult. Full setup & cleanup included. Get a free quote from Hibachi Connect today.`,
  // 1 — what's included + conversion
  (city, state) => `Hibachi at home in ${city}, ${state} — our chefs bring the grill, ingredients, and live-fire entertainment to your backyard. From $55/adult, $550 minimum. Get a free quote now.`,
  // 2 — city + comparison angle
  (city, state) => `Skip the restaurant. Hibachi Connect sends a certified teppanyaki chef to your ${city} home — everything provided, full cleanup included. Starting at $55/person in ${state}.`,
  // 3 — social proof + local
  (city, state) => `Rated 5 stars across 200+ events. Private hibachi chef in ${city}, ${state} — live cooking, fire tricks, and fresh teppanyaki at your home. From $55/adult with $550 minimum.`,
  // 4 — occasion + urgency
  (city, state) => `Planning a party in ${city}? A private hibachi chef brings the grill, food, and entertainment to your ${state} home. $55/adult · $550 min · Setup + cleanup included.`,
]

// 5 OG description variants (separate rotation for richer social previews)
const OG_DESC_TEMPLATES = [
  (city, state) => `A certified hibachi chef comes to your ${city} home with a full teppan grill, fresh ingredients, and live fire cooking. Starting at $55/adult. Book Hibachi Connect today.`,
  (city, state) => `Private teppanyaki at your ${city}, ${state} home — fire tricks, flying shrimp, and freshly cooked meals from $55/adult. Full setup & cleanup. Book Hibachi Connect.`,
  (city, state) => `Hibachi Connect brings restaurant-quality teppanyaki to your ${city} backyard. Certified chefs, premium ingredients, $55/adult. Get a free quote in minutes.`,
  (city, state) => `Your next ${city} party just got upgraded. A private hibachi chef handles everything — grill, food, entertainment, cleanup. From $55/adult. Book with Hibachi Connect.`,
  (city, state) => `Live hibachi cooking at your ${city} home. Our chefs perform, cook, and clean up — all for $55/adult with a $550 event minimum. Serving all ${state} areas.`,
]
...
export function generateCityMeta(cityData) {
  ...
  return {
    ...
    twitter: {
      ...
      description: `Book a certified hibachi chef in ${cityName}. $55/adult · full setup & cleanup · free quote. Hibachi Connect serves all ${stateName}.`,
    },
  }
}
...
export function generateLocalBusinessSchema(cityData) {
  ...
  return {
    ...
    description:`Private hibachi chef service in ${cityName}, ${stateName}. We bring a live teppanyaki grill, fresh ingredients, and full entertainment to your home or event venue. Starting at $55/adult.`,
    areaServed: {
      '@type': 'City',
      name:    cityName,
