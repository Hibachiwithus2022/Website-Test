...
export function generateCityMeta(cityData) {
  ...
  // Defensive CAD conversion for any Canadian city relying on the generic
  // US-priced fallback templates below instead of an explicit meta override.
  const toCad = (s) => s.replace(/\$60\b/g, '$78 CAD').replace(/\$600\b/g, '$780 CAD')

  const title       = metaTitle       ?? TITLE_TEMPLATES[v](cityName, stateName)
  const description = metaDescription ?? DESC_TEMPLATES[v](cityName, stateName)
  const ogDescRaw    = OG_DESC_TEMPLATES[v](cityName, stateName)

  return {
    title:       isOntario && !metaTitle       ? toCad(title)       : title,
    description: isOntario && !metaDescription ? toCad(description) : description,
    keywords: [
      `hibachi ${cityName}`,
    ...
    og: {
      ...
      description: isOntario ? toCad(ogDescRaw) : ogDescRaw,
