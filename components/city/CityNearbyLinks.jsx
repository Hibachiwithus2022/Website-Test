...
export default function CityNearbyLinks({ cityName, stateName, stateSlug, nearbyMajorCities = [], nearbyCities = [] }) {
  ...
  return (
    <section style={{ background: '#1A1209', padding: '4rem 1.5rem', borderTop: '1px solid rgba(245,239,224,0.05)' }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div style={{ display: 'grid', gap: '3rem', alignItems: 'start' }} className="md:grid-cols-[1fr_auto]">
          ...
          <div>
            ...
            <h2 className="font-display" style={{ fontSize: 'clamp(1.8rem,4vw,2.6rem)', color: '#F5EFE0', lineHeight: 1.05, marginBottom: '2rem' }}>
            ...
            <div style={{ display: 'grid', gap: '0.75rem' }} className="sm:grid-cols-2 lg:grid-cols-4">
              {displayCities.map((city) => (
                ...
                >
                  ...
                  <div style={{ fontWeight: 600, color: '#F5EFE0', fontSize: '0.95rem' }}>{city}</div>
                  <div style={{ fontSize: '0.72rem', color: 'rgba(245,239,224,0.38)', marginTop: '0.15rem' }}>{stateName}</div>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(245,239,224,0.4)', marginBottom: '1.25rem' }}>
              Quick Links
            </div>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
              <li>
                <Link href="/" style={{ fontSize: '0.88rem', color: 'rgba(245,239,224,0.65)', textDecoration: 'none', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <span style={{ color: '#C8102E' }}>›</span> Homepage
                </Link>
              </li>
              <li>
                <Link href="/locations" style={{ fontSize: '0.88rem', color: 'rgba(245,239,224,0.65)', textDecoration: 'none', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <span style={{ color: '#C8102E' }}>›</span> All Locations
                </Link>
              </li>
              <li>
                <Link href={`/locations/${stateSlug}`} style={{ fontSize: '0.88rem', color: 'rgba(245,239,224,0.65)', textDecoration: 'none', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <span style={{ color: '#C8102E' }}>›</span> {stateName} Page
                </Link>
              </li>
              <li>
                <a href="#pricing" style={{ fontSize: '0.88rem', color: 'rgba(245,239,224,0.65)', textDecoration: 'none', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
